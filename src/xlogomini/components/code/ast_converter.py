...
class AstConverter(object):
    ...
    def __expand_code_block(self, indent, code_block, tokens):
        for block in code_block:
            ...
            # setpc
            elif block_type == 'setpc':
                tokens.append('setpc')
                tokens.append(block['value'])
            # For loops
    ...
    def __is_command(self, block_type):
        if block_type == 'fd': return True
        if block_type == 'bk': return True
        if block_type == 'lt': return True
        if block_type == 'rt': return True
        return False
